...
3개 언어를 지원합니다: 영어(en), 한국어(ko), 일본어(ja). 아래 import 예시는 `src/` 바로 아래 파일을 기준으로 합니다.

```tsx
import { I18nProvider, useTranslation } from "./ui/i18n";
...
- 번역 키는 `src/ui/i18n/locales/` 디렉토리에서 관리
...
4. **최소 의존성** - UI 런타임은 앱의 react/react-dom과 clsx를 사용
